...
export default function CartModal() {
  ...
  // Conditionally fetch cart data when session is available
  const { data: products, isLoading } = useQuery(['getCart', session?.user?.id], async () => {
    if (!session?.user?.id) return; // If no userId available, don't fetch
  ...
  const newOrder = {
    ...
    //@ts-ignore
    productId: !!products && !!products.products ? products.products.map(p => p.id) : [],
    quantity: 1,
